...
void M_AdjustSliders (int dir)
{
	...
	*/
	...
	case 10:
		...
		if (absPitch > 0.62) absPitch = 0.62;
...
// cursor
	...
	/* Removed in favor of separate m_pitch m_yaw
	ypos += 8;
	M_Print (16, ypos, "           Mouse Speed");
	r = (sensitivity.value - 1)/10;
	M_DrawSlider (220, ypos, r);
	*/

	ypos += 8;
	M_Print (16, ypos, "       CD Music Volume");
	r = bgmvolume.value;
	M_DrawSlider (220, ypos, r);
	...
	ypos += 8;
	M_Print (16, ypos, "            Always Run");
	M_DrawCheckbox (220, ypos, cl_forwardspeed.value > 200);

	ypos += 8;
	M_Print (16, ypos, "         Invert Look Y");
	M_DrawCheckbox (220, ypos, m_pitch.value < 0);

	/* Removed in favor of dual analog control
	ypos += 8;
	M_Print (16, ypos, "            Lookspring");
	M_DrawCheckbox (220, ypos, lookspring.value);

	ypos += 8;
	M_Print (16, ypos, "            Lookstrafe");
	M_DrawCheckbox (220, ypos, lookstrafe.value);
	*/

	ypos += 8;
	M_Print (16, ypos, "    Look Sensitivity X");
	r = (m_yaw.value - 0.12) / (0.62 - 0.12);
	M_DrawSlider (220, ypos, r);

	ypos += 8;
	M_Print (16, ypos, "    Look Sensitivity Y");
	r = (fabs(m_pitch.value) - 0.12) / (0.62 - 0.12);
	M_DrawSlider (220, ypos, r);

	ypos += 8;
	M_Print (16, ypos, "          Swap analogs");
	M_DrawCheckbox (220, ypos, swap_analogs.value);

	ypos += 8;
	M_Print (16, ypos, "    Weapon Wheel Stick");
	if (weapwheel_use_move.value) M_Print(220, ypos, "move");
	else M_Print(220, ypos, "look");

	ypos += 8;
	M_Print (16, ypos, "             Crosshair");
	M_DrawCheckbox (220, ypos, crosshair.value);

	ypos += 8;
	M_Print (16, ypos, "               Autoaim");
	if (sv_aim.value == 0) M_Print(220, ypos, "off");
	if (sv_aim.value == 1) M_Print(220, ypos, "vertical");
	if (sv_aim.value == 2) M_Print(220, ypos, "full");

	ypos += 8;
	M_Print (16, ypos, "         Center weapon");
	M_DrawCheckbox (220, ypos, center_weapon.value);
...
void M_Cheats_Draw (void)
{
	...
	M_DrawTransPic (16, 4, Draw_CachePic ("gfx/qplaque.lmp") );
	p = Draw_CachePic ("gfx/p_option.lmp");
	int xpos = 8 + (320 - p->width) / 2;
	M_DrawPic ( (320-p->width)/2, 4, p);
	M_Print(xpos, ypos, "Cheats");
	ypos += 16;
	M_Print(xpos, ypos, "Give All");
	ypos += 8;
	M_Print(xpos, ypos, "Activate Quad");
	ypos += 8;
	M_Print(xpos, ypos, "Toggle God Mode");
	ypos += 8;
	M_Print(xpos, ypos, "Toggle No-Clip");
	ypos += 8;
	M_Print(xpos, ypos, "Toggle No-Target");
	ypos += 8;

	M_DrawCharacter (xpos - 8, 32 + 16 + cheats_cursor*8, 12+((int)(realtime*4)&1));
}

void M_Cheats_Key (int k)
{
	switch (k)
	{
	case RG_B:
	...
	case K_RIGHTARROW:
		...
		else if (cheats_cursor == 1)
		{
			Cbuf_AddText("impulse 255\n");
		}
		else if (cheats_cursor == 2)
		{
			Cbuf_AddText("god\n");
		}
		else if (cheats_cursor == 3)
		{
			Cbuf_AddText("noclip\n");
		}
		else if (cheats_cursor == 4)
		{
	...
	case K_UPARROW:
		...
		S_LocalSound ("misc/menu1.wav");
		if (cheats_cursor < 0)
			cheats_cursor = CHEATS_ITEMS - 1;
		break;

	case K_DOWNARROW:
		cheats_cursor += 1;
		S_LocalSound ("misc/menu1.wav");
		if (cheats_cursor >= CHEATS_ITEMS)
			cheats_cursor = 0;
		break;
...
void M_Keys_Key (int k)
{
	...
	case RG_SELECT:		// delete bindings
		S_LocalSound ("misc/menu2.wav");
		M_UnbindCommand (bindnames[keys_cursor][0]);
		break;
	}
