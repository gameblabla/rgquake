...
// Added to check if essential files exist during bootup
...
// shp: moved this from before pak to after.
//
	search = Hunk_Alloc (sizeof(searchpath_t));
...
//
	...
	if (i && i < com_argc-1)
	{
		// we have a basedir set, but for OPK builds we want to keep
		// another basedir
		// COM_AddGameDirectory (va("%s/"GAMENAME, host_parms.basedir) );
